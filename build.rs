...
const MODULE_PATH: &[&str] = &["signal_message", "lib"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        let source =
            fs::read_to_string(&source_path).expect("read ordinary Message Interface source");
        ...
        .expect("assemble authority-approved ordinary Message Interface transaction");
        let rust = rust_logos();
        let type_paths = MessageRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project ordinary Message Interface from the verified transaction")
            .write_or_check("SIGNAL_MESSAGE_UPDATE_INTERFACE_ARTIFACTS")
            .expect("checked-in ordinary Message Interface source and Rust projection are fresh");
        CargoEthosSourceMetadata::new("signal-message")
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("Message bootstrap catalog is complete")
}
